    ...
    pub async fn record_context_provided(&self, context: ProvidedContext) -> Result<String> {
        ...
        conn.execute(
            ...
            libsql::params![
                ...
                context.context_id,
                task_hash,
                task_keywords.and_then(|k| serde_json::to_string(&k).ok()),
                context.task_type.map(|t| t.to_string()),
                context.work_phase.map(|p| p.to_string()),
                context.file_types.and_then(|f| serde_json::to_string(&f).ok()),
                context.error_context.map(|e| e.to_string()),
                context.related_technologies.and_then(|t| serde_json::to_string(&t).ok()),
                now,
                now,
            ],
        )
