#!/usr/bin/env python3
"""BootstrapFewShot optimization for validate_correctness signature with Tier 3 metrics.

Tier 3 optimizer - uses improved LLM-as-judge metric for correctness quality.

Addresses Tier 2 stagnation (77.5% → 77.5%, 0% improvement) by replacing
binary boolean metric with multi-dimensional semantic quality evaluation.

# Usage

```bash
# Test run
python bootstrap_validate_correctness_tier3.py --test-mode --output /tmp/validate_correctness_tier3_test.json

# Full optimization
python bootstrap_validate_correctness_tier3.py --max-demos 8 --output /tmp/validate_correctness_tier3.json
```

# Expected Improvement

Tier 2 baseline: 77.5% (binary metric)
Tier 3 target: 85%+ (+10% improvement with quality-based metric)

Better metrics unlock optimization signal - measures nuance, not just binary correctness.
"""
...
from reviewer_module import ValidateCorrectness
from semantic_metrics_tier3 import correctness_quality_metric
...
def load_validate_correctness_data(data_dir: Path) -> List[dspy.Example]:
    """Load training data for validate_correctness signature only."""
    json_path = data_dir / "validate_correctness.json"
    ...
    for item in raw_data:
        ...
        if not all(field in inputs for field in ["implementation"]):
            logger.warning(f"Skipping example missing required inputs")
            continue
        if not all(field in outputs for field in ["is_correct"]):
    ...
    logger.info(f"Loaded {len(examples)} validate_correctness examples")
    return examples
...
class ValidateCorrectnessModule(dspy.Module):
    """Minimal module wrapping only the validate_correctness signature."""

    def __init__(self):
        super().__init__()
        self.predictor = dspy.ChainOfThought(ValidateCorrectness)

    def forward(self, implementation: str, code_sample: str = ''):
        """Validate implementation correctness."""
        return self.predictor(implementation=implementation, code_sample=code_sample)
...
def optimize_with_bootstrap(
...
) -> ValidateCorrectnessModule:
    """Optimize validate_correctness using BootstrapFewShot with Tier 3 metrics."""
    logger.info("Initializing ValidateCorrectnessModule for Tier 3 optimization")
    module = ValidateCorrectnessModule()
    ...
    teleprompter = BootstrapFewShot(
        metric=correctness_quality_metric,  # Tier 3: Multi-dimensional quality evaluation
    ...
    logger.info(f"  metric: correctness_quality_metric (LLM-as-judge)")
...
def evaluate_module(
    ...
    for example in test_data:
        try:
            pred = module(**{k: getattr(example, k) for k in ['implementation']})
            score = correctness_quality_metric(example, pred)
            scores.append(score)
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize validate_correctness signature with BootstrapFewShot (Tier 3 - Quality Metrics)"
    )
    parser.add_argument(
        "--max-demos",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="/tmp/validate_correctness_tier3.json",
    ...
    all_data = load_validate_correctness_data(data_dir)
    ...
    baseline_module = ValidateCorrectnessModule()
    ...
    results = {
        ...
        "signature": "validate_correctness",
        "optimizer": "BootstrapFewShot",
        "tier": 3,
        "metric": "correctness_quality_metric (LLM-as-judge)",
        "config": {
