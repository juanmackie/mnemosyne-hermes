#!/usr/bin/env python3
"""BootstrapFewShot optimization for validate_completeness signature.
...
```bash
# Test run
python bootstrap_validate_completeness.py --test-mode --output /tmp/validate_completeness_bootstrap_test.json

# Full optimization
python bootstrap_validate_completeness.py --max-demos 8 --output /tmp/validate_completeness_bootstrap.json
```
...
def optimize_with_bootstrap(
    ...
    """Optimize validate_completeness using BootstrapFewShot.

    Args:
        training_data: Training examples
        max_bootstrapped_demos: Max bootstrapped demonstrations (default: 8)
        max_labeled_demos: Max labeled demonstrations (default: 4)
        test_mode: If True, reduce demos for faster testing
    ...
    teleprompter = BootstrapFewShot(
        ...
        max_rounds=1,  # Single-round bootstrapping
        max_errors=5,  # Allow some errors during bootstrapping
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize validate_completeness signature with BootstrapFewShot (Tier 2)"
    )
    parser.add_argument(
        "--max-demos",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="/tmp/validate_completeness_bootstrap.json",
    ...
    # Split 20/80 (INVERTED from traditional ML - more validation data for prompt optimization)
