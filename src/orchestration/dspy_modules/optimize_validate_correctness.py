#!/usr/bin/env python3
"""MIPROv2 optimization for validate_correctness signature.

Focused single-signature optimization to avoid dispatch ambiguity.
Optimizes only the correctness validation predictor.

# Usage

```bash
# Test run (5 trials)
python optimize_validate_correctness.py --test-mode --output /tmp/validate_correctness_test.json

# Full optimization (20-30 trials)
python optimize_validate_correctness.py --trials 25 --output validate_correctness_v1.json
```
...
from semantic_metrics_fast import correctness_metric
...
def load_validate_correctness_data(data_dir: Path) -> List[dspy.Example]:
    ...
    Returns:
        List of DSPy Examples with inputs (implementation, code_sample) and
...
def optimize_validate_correctness(
    training_data: List[dspy.Example],
    num_trials: int = 25,
    test_mode: bool = False
) -> ValidateCorrectnessModule:
    """Optimize validate_correctness using MIPROv2.
    ...
    Returns:
        Optimized module
    """
    logger.info("Initializing ValidateCorrectnessModule for optimization")
    ...
    teleprompter = MIPROv2(
        metric=correctness_metric,  # FOCUSED METRIC - no dispatch ambiguity
        auto=None,
        num_candidates=10,
...
def evaluate_module(
    ...
    for example in test_data:
        try:
            pred = module(implementation=example.implementation, code_sample=getattr(example, 'code_sample', ''))
            score = correctness_metric(example, pred)
            scores.append(score)
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize validate_correctness signature with MIPROv2"
    )
    parser.add_argument(
        "--trials",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="validate_correctness_v1.json",
    ...
    # Optimize
    optimized_module = optimize_validate_correctness(train_data, args.trials, args.test_mode)
