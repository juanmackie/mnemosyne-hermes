...
Provides standard interface for multi-agent orchestration.
"""
...
@dataclass
class WorkItem:
    """Work item structure for agent tasks."""
    id: str
    description: str
    phase: str
    priority: int
...
@dataclass
class WorkResult:
    """Work result from agent execution."""
    success: bool
    data: str | None = None
...
class AgentExecutionMixin:
    """
    Mixin providing standard execute_work interface for agent orchestration.

    All agents should inherit from this mixin to provide consistent
    interface for work execution.
    """
    ...
    async def execute_work(self, work_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            work_dict: Work item dictionary
