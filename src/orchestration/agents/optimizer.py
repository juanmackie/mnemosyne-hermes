...
Provide reasoning for your optimization decisions."""
    ...
    async def _execute_work_item(self, work_item: WorkItem) -> WorkResult:
        """
        Execute work item (context optimization).

        Implements the AgentExecutionMixin interface for agent
        orchestration.
...
Keep your response concise and structured."""
        ...
        # Infer file types
        file_types = []
        if 'rust' in task_lower or '' in task_lower:
            file_types.append('')
...
async def main():
    """Run optimizer agent as independent process."""
    import argparse
    import asyncio
    import httpx
    import signal
    ...
    parser.add_argument("--agent-id", default="optimizer", help="Agent ID")
    ...
    logger.info(f"Starting Optimizer Agent (ID: {args.agent_id})")
    ...
    try:
        logger.info("Optimizer agent running (press Ctrl+C to stop)")
        ...
        logger.info("Optimizer agent stopped")
