...
Always follow best practices and validate your work before marking it complete."""
    ...
    async def _execute_work_item(self, work_item: WorkItem) -> WorkResult:
        """
        Execute work item for agent orchestration.
        ...
        """
        logger.info(f"Received work item: {work_item.id} (phase: {work_item.phase})")
        ...
        try:
            ...
            # Return concise error for agent bridge
            return WorkResult(
                success=False,
                error=format_error(error_context)
            )
