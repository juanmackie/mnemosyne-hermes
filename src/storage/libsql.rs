...
impl LibsqlStorage {
    /// Shared paginated listing with a stable, offset-based window (private
    /// helper backing both `list_memories` and `list_memories_page`).
    /// Ordering always includes the unique `id` tiebreaker so consecutive
    /// pages do not overlap or skip rows when timestamps collide.
...
mod link_type_tests {
    ...
    fn link(target: MemoryId, link_type: LinkType) -> MemoryLink {
        MemoryLink {
            target_id: target,
            link_type,
            strength: 0.9,
    ...
    async fn repair_widens_an_old_database_and_keeps_rows_and_defaults() {
        ...
        let query = "SELECT COUNT(*) FROM sqlite_master WHERE type = 'index' \
                 AND tbl_name = 'memory_links' AND sql IS NOT NULL";
        let recreated = scalar_i64(&store, query).await;
        assert!(
            recreated >= 2,
