...
has_import_command() {
  ...
  local cli_target="$TMP/cli-import.db"
  local text_target="$TMP/text-import.db"
...
MNEMOSYNE_DB_PATH="$project_target" RUST_LOG=error "$BIN" import --from "$source" \
  --namespace project:hermes --format json >"$TMP/import-project.json" 2>/dev/null || return 1
...
"$BIN" --db-path "$cli_target" import --from "$source" \
  --namespace global --format json >"$TMP/import-cli-path.json" 2>/dev/null || return 1
"$BIN" --db-path "$cli_target" list --namespace global --format json 2>/dev/null \
  | grep -q 'local-only storage' || return 1
touch "$TMP/import-cli-path.ok"
"$BIN" --db-path "$text_target" import --from "$source" \
  --namespace agent:hermes >"$TMP/import-text.txt" 2>/dev/null || return 1
grep -q 'Imported 7 of 9 rows' "$TMP/import-text.txt" || return 1
grep -q 'Tables:' "$TMP/import-text.txt" || return 1
touch "$TMP/import-text.ok"
...
import_cli_db_path_is_honored() {
  [[ -f "$TMP/import-cli-path.ok" ]]
}

import_text_report_is_human_readable() {
  [[ -f "$TMP/import-text.ok" ]]
}
...
check importer_cli_path import_cli_db_path_is_honored
check importer_text import_text_report_is_human_readable
