...
Status: IMPLEMENTATION APPLIED (repo-level retirement executed per full retirement authorization; planning deliverable preserved as design artifact). Source files deleted (`src/` retired; preserved at archive ref `09a6973`/`ba6fe984`); build/release adapter removed (`Makefile` targets retired, `scripts/rebuild-and-update-install.sh` retired, `.github/workflows/ci.yml` Rust steps retired); docs updated (`README.md` archive/quickstart note, `ARCHITECTURE.md` archive/reference note, `AGENTS.md` verification/archive updates). Contracts preserved: `memory.provider: mnemosyne` / `mnemosyne-rust`, namespace `agent:hermes`, DB path, provider identity, tool names, persisted identifiers.
