#!/usr/bin/env python3
"""Evaluate real-query retrieval through the public MCP stdio surface."""
...
def one_query(binary: Path, db: Path, namespace: str, item: dict,
    ...
    if proc.returncode:
        raise RuntimeError(
            f"MCP recall failed for {item['query']!r}: {proc.stderr[-1000:]}")
    ...
    if payload is None:
        raise RuntimeError(
            f"MCP recall returned no JSON payload for {item['query']!r}")
...
def main() -> int:
    ...
    output = {"dataset": str(args.dataset), "items": len(items), "mode": "flat"}
